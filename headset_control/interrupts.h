...
int cb_get_gyro_data(bool &state, libusb_device_handle* &dev_handle, libusb_context* &ctx){
//    cout << "\nSending Interrupt to Headset" << endl;
	int res = 0;
...
//	        for (int i = 0; i < actual_length; i++) {
		printf("Data:\n");
				//Timer Value
				value = (data[30] | (data[31] << 8));
				printf("Rapid 01: %d ", value);
...
*/				value = (data[36] | (data[37] << 8));
				...
				printf("Facing Forwards or Upside Down: %d ", value);/**/
...
/*				value = (data[50] | (data[51] << 8));
				printf("Unknown Val: %d ", value);
				value = (data[52] | (data[53] << 8));
				printf("Rapid Val: %d ", value);*/

				//Switching Value of 1/0.
				printf("\n");
				value = (data[54] | (data[55] << 8));
				printf("Rapid 01: %d ", value);
/*				value = (data[56] | (data[57] << 8));
				printf("Random Val: %d ", value);*/

				//Counter Value
/*				value = (data[58] | (data[59] << 8));
				printf("1ms Counter: %d ", value);*/
...
*/				printf("\n");
				value = (data[126] | (data[127] << 8));
				printf("Rapid 01: %d ", value);
				value = (data[132] | (data[133] << 8));
				printf("Pitch Up/Down: %d ", value);
				value = (data[134] | (data[135] << 8));
				printf("Roll Left/Right: %d ", value);
				value = (data[136] | (data[137] << 8));
				printf("Facing Forwards or Upside Down: %d", value);
